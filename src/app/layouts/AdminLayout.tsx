import { BarChart3, DollarSign, HandCoins, Layers, Megaphone, MessageSquare, ServerCog, ShieldAlert, ShieldCheck, Store, TrendingUp } from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { label: "Yêu cầu rút tiền", icon: HandCoins, to: "/admin/marketplace/payouts", payouts: true },
...
export default function AdminLayout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "#F1F5F9", fontFamily: "'Inter', sans-serif", color: "#111827" }}>
        ...
        <nav className="flex-1 space-y-0.5 px-3 overflow-y-auto">
          {NAV_ITEMS.map((item) => {
            ...
            const active = Boolean(
              ("payouts" in item && item.payouts && location.pathname.startsWith("/admin/marketplace/payouts"))
              || ("marketplace" in item && item.marketplace && location.pathname.startsWith("/admin/marketplace") && !location.pathname.startsWith("/admin/marketplace/payouts")),
            );
      ...
      <main className="min-w-0 flex-1 overflow-y-auto"><header className="flex h-14 items-center border-b border-slate-200 bg-white/95 px-8"><div><p className="text-sm font-bold text-slate-900">{location.pathname.startsWith("/admin/marketplace/payouts") ? "Yêu cầu rút tiền" : "Duyệt Quiz Pack"}</p><p className="text-[11px] text-slate-400">{location.pathname.startsWith("/admin/marketplace/payouts") ? "Marketplace payout operations" : "Marketplace moderation"}</p></div></header><Outlet /></main>
    </div>
  );
}
