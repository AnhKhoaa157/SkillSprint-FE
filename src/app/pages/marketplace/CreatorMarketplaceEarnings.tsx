...
function StatCard({ label, value, tone }: { label: string; value: number; tone: "orange" | "amber" | "slate" | "green" }) {
  ...
  return <div className={`rounded-2xl border p-4 ${colors[tone]}`}><p className="text-[10px] font-black uppercase tracking-[0.14em] opacity-70">{label}</p><p className="mt-2 text-xl font-black tabular-nums sm:text-2xl">{money.format(value)} <span className="text-xs sm:text-sm">Coin</span></p></div>;
}
