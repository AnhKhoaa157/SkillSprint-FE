...
function StatCard({ label, value, tone }: { label: string; value: number; tone: "orange" | "amber" | "slate" | "green" }) {
  ...
  return <div className={`rounded-2xl border p-4 ${colors[tone]}`}><p className="text-[10px] font-black uppercase tracking-[0.14em] opacity-70">{label}</p><p className="mt-2 text-xl font-black tabular-nums sm:text-2xl">{formatVnd(value)}</p></div>;
}
...
export default function CreatorMarketplaceEarnings() {
  ...
  const requestPayout = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isInteger(parsedAmount) || parsedAmount <= 0) {
      toast.error("Số tiền rút phải là số nguyên lớn hơn 0 VND.");
      return;
    }
    if (!earnings || parsedAmount > earnings.availableAmount) {
      toast.error("Số tiền rút vượt quá số dư có thể rút.");
      return;
    }
    if (!destination) {
  ...
  return <main className="relative isolate min-h-full overflow-hidden bg-[#F7F8FA] p-4 text-slate-900 sm:p-7 lg:p-9">
    <div className="pointer-events-none absolute inset-0 -z-20 bg-[radial-gradient(circle_at_6%_4%,rgba(255,237,223,0.75),transparent_27%),radial-gradient(circle_at_94%_15%,rgba(255,246,234,0.7),transparent_22%)]" />
    <div className="mx-auto max-w-6xl">
      <section className="relative overflow-hidden rounded-[2rem] border border-white bg-white/90 p-5 shadow-[0_20px_60px_rgba(71,50,35,0.07)] backdrop-blur-xl sm:p-7">
        <div className="pointer-events-none absolute -right-16 -top-24 h-64 w-64 rounded-full bg-orange-100/70 blur-3xl" />
        <div className="relative grid gap-7 lg:grid-cols-[minmax(0,0.9fr)_minmax(420px,1.1fr)] lg:items-end">
          <div><span className="inline-flex items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-[#FF6B00]"><WalletCards className="h-3.5 w-3.5" />Creator finance</span><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><h1 className="text-3xl font-black tracking-[-0.035em] text-slate-950 sm:text-4xl">Thu nhập & rút tiền</h1><button type="button" onClick={load} className="inline-flex h-10 items-center gap-2 rounded-xl border border-orange-200 bg-white px-3.5 text-sm font-bold text-[#FF6B00] shadow-sm transition hover:bg-orange-50"><RefreshCw className="h-4 w-4" />Làm mới</button></div><p className="mt-3 max-w-xl text-sm leading-6 text-slate-500">Theo dõi doanh thu, cập nhật nơi nhận và gửi yêu cầu rút VND để Admin xử lý.</p></div>
          <div className="grid gap-3 sm:grid-cols-2"><StatCard label="Có thể rút" value={earnings.availableAmount} tone="orange" /><StatCard label="Đang chờ" value={earnings.pendingAmount} tone="amber" /><StatCard label="Đang giữ để rút" value={earnings.reservedAmount} tone="slate" /><StatCard label="Đã nhận" value={earnings.paidAmount} tone="green" /></div>
        </div>
      </section>

      <div className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,0.85fr)]">
        <section className="rounded-[1.75rem] border border-white bg-white/95 p-5 shadow-[0_16px_45px_rgba(71,50,35,0.06)] sm:p-6"><div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-orange-50 text-[#FF6B00] ring-1 ring-orange-100"><Landmark className="h-5 w-5" /></span><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#FF6B00]">Bước 1 · Nơi nhận tiền</p><h2 className="mt-1 font-black text-slate-950">Thông tin nhận tiền</h2><p className="mt-1 text-sm leading-5 text-slate-500">Admin dùng QR để đối chiếu trước khi chuyển khoản thủ công.</p></div></div>
          <form className="mt-6 space-y-4" onSubmit={saveDestination}>
            ...
            <div className="flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">{destination?.qrViewUrl && !qrFile ? <a className="inline-flex w-fit items-center gap-1.5 text-sm font-bold text-[#FF6B00] hover:underline" href={destination.qrViewUrl} target="_blank" rel="noreferrer">Xem ảnh QR hiện tại <ExternalLink className="h-3.5 w-3.5" /></a> : <p className="text-xs leading-5 text-slate-400">Cần ảnh QR mới mỗi lần cập nhật thông tin.</p>}<button type="submit" disabled={savingDestination} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-bold text-white transition hover:bg-[#FF6B00] disabled:cursor-not-allowed disabled:opacity-50">{savingDestination ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <BadgeCheck className="h-4 w-4" />}Lưu thông tin nhận tiền</button></div>
          </form>
        </section>
        <section className="overflow-hidden rounded-[1.75rem] border border-white bg-white/95 shadow-[0_18px_52px_rgba(71,50,35,0.08)] xl:sticky xl:top-6"><div className="bg-slate-950 px-5 py-5 text-white sm:px-6"><div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white/10 text-orange-300 ring-1 ring-white/15"><BanknoteArrowDown className="h-5 w-5" /></span><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-orange-300">Bước 2 · Rút VND</p><h2 className="mt-1 font-black">Tạo yêu cầu rút</h2><p className="mt-1 text-sm leading-5 text-slate-300">Số tiền sẽ được giữ lại đến khi yêu cầu được xử lý.</p></div></div></div><form className="p-5 sm:p-6" onSubmit={requestPayout}><div className="rounded-2xl border border-orange-100 bg-orange-50/70 p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#D94E00]">Số dư có thể rút</p><p className="mt-2 text-3xl font-black tracking-[-0.03em] text-slate-950 tabular-nums">{formatVnd(earnings.availableAmount)}</p></div><label className="mt-5 block text-sm font-bold text-slate-700">Số tiền muốn rút (VND)<input type="number" min="1" step="1" value={amount} onChange={event => setAmount(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-slate-200 px-4 text-xl font-black tabular-nums outline-none transition focus:border-orange-300 focus:ring-4 focus:ring-orange-100" placeholder="0" /></label><p className="mt-2 text-xs leading-5 text-slate-500">Nhập số nguyên VND, không vượt quá số dư hiện có.</p><button type="submit" disabled={requestingPayout || !destination || earnings.availableAmount <= 0} className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#FF6B00] px-5 text-sm font-bold text-white shadow-[0_10px_22px_rgba(255,107,0,0.2)] transition hover:bg-[#E85F00] disabled:cursor-not-allowed disabled:opacity-45">{requestingPayout ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <BanknoteArrowDown className="h-4 w-4" />}Gửi yêu cầu rút tiền</button>{!destination && <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2.5 text-xs leading-5 text-amber-800">Bạn cần lưu QR nhận tiền trước khi có thể rút VND.</p>}</form></section>
      </div>

      <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between gap-4"><div><h2 className="font-black text-slate-950">Lịch sử yêu cầu rút</h2><p className="mt-1 text-sm text-slate-500">Trạng thái được Admin cập nhật sau khi kiểm tra và chuyển khoản.</p></div><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600">{payouts.length} yêu cầu</span></div><div className="mt-5 space-y-3">{payouts.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 px-5 py-10 text-center text-sm text-slate-500">Chưa có yêu cầu rút tiền nào.</div> : payouts.map(payout => { const status = PAYOUT_STATUS[payout.status]; return <article key={payout.payoutId} className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-slate-50/70 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-black text-slate-900">{formatVnd(payout.requestedAmount)}</p><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${status.className}`}>{status.label}</span></div><p className="mt-2 text-xs text-slate-500">{payout.bankName} · {payout.accountNumberMasked ?? "Tài khoản đã lưu"} · {date(payout.createdAt)}</p>{payout.rejectionReason && <p className="mt-2 text-xs font-semibold text-rose-700">Lý do: {payout.rejectionReason}</p>}</div>{payout.externalTransferReference && <span className="text-xs font-semibold text-slate-500">Mã giao dịch: {payout.externalTransferReference}</span>}</article>; })}</div></section>
    </div>
  </main>;
}
