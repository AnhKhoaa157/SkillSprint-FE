...
function LearningPackOutline({ chapters, questionCount }: { chapters: MarketplaceChapter[]; questionCount: number }) {
  return <section className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_14px_36px_rgba(15,23,42,0.05)] sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#FF6B00]">Lộ trình học</p><h2 className="mt-1 text-xl font-black tracking-[-0.025em] text-slate-950">Nội dung gói học</h2></div><span className="rounded-xl border border-orange-100 bg-orange-50 px-3 py-2 text-xs font-bold text-[#C2410C]">{chapters.length} chương</span></div>
    <div className="mt-5 space-y-2.5">{chapters.map((chapter, index) => <details key={chapter.chapterId} open={index === 0} className="group overflow-hidden rounded-2xl border border-slate-100 bg-slate-50/60 transition hover:border-orange-100 hover:bg-orange-50/40"><summary className="flex cursor-pointer list-none items-center gap-3 p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-orange-300"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-xs font-black text-[#FF6B00] shadow-sm ring-1 ring-orange-100">{index + 1}</span><span className="min-w-0 flex-1"><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">Chương {index + 1}</span><span className="mt-0.5 block truncate text-sm font-bold text-slate-900">{chapter.title}</span></span><ChevronDown className="h-4 w-4 shrink-0 text-slate-400 transition duration-200 group-open:rotate-180" /></summary>{chapter.summary && <p className="border-t border-slate-100 px-4 py-3 pl-[4.5rem] text-sm leading-6 text-slate-600">{chapter.summary}</p>}</details>)}</div>
    <div className="mt-5 flex items-center gap-3 rounded-2xl border border-orange-100 bg-orange-50/70 p-3.5"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-[#FF6B00] shadow-sm"><FileQuestion className="h-4 w-4" /></span><p className="text-sm leading-5 text-slate-600"><b className="text-slate-900">{questionCount} câu hỏi</b> sẽ mở khi bạn bắt đầu Full Pack Challenge.</p></div>
  </section>;
}
...
export function MyPackLearningPage() {
  ...
  return <Shell><div className="mx-auto max-w-6xl"><Link to="/my-packs" className="inline-flex min-h-11 items-center gap-1.5 text-sm font-bold text-[#FF6B00] transition hover:text-[#C2410C]"><ArrowLeft className="h-4 w-4" />Gói của tôi</Link><section className="relative mt-3 overflow-hidden rounded-[2rem] border border-orange-100 bg-[radial-gradient(circle_at_85%_15%,rgba(255,187,118,0.28),transparent_28%),linear-gradient(125deg,#FFF8F1_0%,#FFFFFF_62%,#FFF2E4_100%)] p-6 shadow-[0_16px_44px_rgba(194,65,12,0.08)] sm:p-8"><div aria-hidden="true" className="pointer-events-none absolute -right-12 -top-14 h-40 w-40 rounded-full border-[24px] border-orange-200/45" /><div className="relative max-w-3xl"><span className="inline-flex rounded-xl border border-orange-100 bg-white/80 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-[#FF6B00]">{pack.subject}</span><h1 className="mt-4 text-3xl font-black tracking-[-0.04em] text-slate-950 sm:text-4xl">{pack.title}</h1>{pack.description && <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">{pack.description}</p>}<div className="mt-5 flex flex-wrap gap-2"><span className="rounded-xl border border-white bg-white/85 px-3 py-2 text-xs font-bold text-slate-600 shadow-sm"><b className="text-slate-950">{pack.chapters.length}</b> chương</span><span className="rounded-xl border border-white bg-white/85 px-3 py-2 text-xs font-bold text-slate-600 shadow-sm"><b className="text-slate-950">{pack.questions.length}</b> câu hỏi</span></div></div></section><div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><LearningPackOutline chapters={pack.chapters} questionCount={pack.questions.length} /><section className="h-fit rounded-[1.75rem] border border-orange-200 bg-[linear-gradient(155deg,#FFF9F3,#FFF1E2)] p-5 shadow-[0_14px_36px_rgba(194,65,12,0.09)] xl:sticky xl:top-24"><div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#FF6B00] text-white shadow-[0_9px_18px_rgba(255,107,0,0.22)]"><Trophy className="h-5 w-5" /></span><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#E85F00]">Thử thách cuối gói</p><h2 className="mt-1 text-lg font-black tracking-[-0.02em] text-slate-950">Full Pack Challenge</h2><p className="mt-1 text-sm leading-5 text-slate-600">Hoàn thành {pack.questions.length} câu hỏi để nhận kết quả.</p></div></div>{!session && <button onClick={start} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#FF6B00] px-5 text-sm font-black text-white shadow-[0_10px_20px_rgba(255,107,0,0.2)] transition hover:-translate-y-0.5 hover:bg-[#E85F00] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200"><Trophy className="h-4 w-4" />Bắt đầu thử thách</button>}{expired && <p className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-sm font-bold leading-5 text-amber-900">Phiên đã hết hạn. Hãy bắt đầu một phiên mới.</p>}{session && !expired && <div className="mt-5 rounded-2xl border border-white bg-white/80 p-3.5"><p className="text-xs font-semibold text-slate-500">Bắt đầu {date(session.startedAt)}</p><div className="mt-3"><Timer expiresAt={session.expiresAt} expire={() => setExpired(true)} /></div></div>}</section></div>{session && !expired && <section className="mt-6 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_14px_36px_rgba(15,23,42,0.05)] sm:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#FF6B00]">Đang làm bài</p><h2 className="mt-1 text-xl font-black text-slate-950">Chọn đáp án cho từng câu hỏi</h2></div><span className="rounded-xl bg-orange-50 px-3 py-2 text-xs font-black text-[#C2410C]">{Object.keys(answers).length}/{session.questions.length} đã trả lời</span></div><Questions questions={session.questions} answers={answers} select={(id, option) => setAnswers(a => ({ ...a, [id]: option }))} /><button onClick={() => setConfirm(true)} disabled={!done} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#FF6B00] px-5 text-sm font-black text-white shadow-[0_10px_20px_rgba(255,107,0,0.2)] transition hover:-translate-y-0.5 hover:bg-[#E85F00] disabled:cursor-not-allowed disabled:opacity-45 disabled:shadow-none sm:w-auto"><Send className="h-4 w-4" />Nộp bài ({Object.keys(answers).length}/{session.questions.length})</button>{!done && <p className="mt-2 text-xs text-slate-500">Bạn cần trả lời mọi câu hỏi trước khi nộp.</p>}</section>}{result && <section className="mt-6 rounded-[1.75rem] border border-emerald-100 bg-emerald-50/70 p-5 sm:p-6"><div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-600 text-white"><CheckCircle2 className="h-5 w-5" /></span><div><h2 className="font-black text-emerald-950">Kết quả thử thách</h2><p className="mt-1 text-sm text-emerald-800">Hoàn thành {date(result.completedAt)}</p></div></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Điểm",result.score],["Đúng",`${result.correctCount}/${result.questionCount}`],["Câu hỏi",result.questionCount],["Thời gian",`${result.durationSeconds}s`]].map(([k,v]) => <div key={String(k)} className="rounded-2xl border border-white bg-white/80 p-3.5"><p className="text-xs font-semibold text-slate-500">{k}</p><p className="mt-1 font-black text-emerald-700">{v}</p></div>)}</div></section>}<Confirm open={confirm} title="Xác nhận nộp bài" text="Bạn đã trả lời đủ tất cả câu hỏi. Bạn muốn gửi bài thử thách?" button="Nộp bài" busy={submitting} close={() => setConfirm(false)} submit={submit} /></div></Shell>;
}
