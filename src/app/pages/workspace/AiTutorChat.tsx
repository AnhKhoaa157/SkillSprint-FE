  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-[#F9FAFB] text-slate-800 rounded-xl overflow-hidden border border-slate-100/80">
      ...
      <div className="flex items-center gap-2 px-3.5 pt-2.5 pb-2 shrink-0 border-b border-slate-100 bg-white">
        <div className="flex h-5 w-5 items-center justify-center rounded-md bg-indigo-50 border border-indigo-100 shrink-0">
        ...
        <span className="shrink-0 inline-flex items-center gap-0.5 rounded-full bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 text-[8px] font-bold text-indigo-600">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3.5 min-h-0 px-3.5 py-3 bg-[#F9FAFB]">
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full py-6 text-center px-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 mb-3 shadow-sm">
            ...
            <div className="mt-4 flex flex-col gap-1.5 w-full max-w-[200px]">
              {["Cột mốc này học gì?", "Mất bao lâu để hoàn thành?"].map((hint, i) => (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[9px] font-medium text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-200 transition-all text-left"
                >
        ...
        {messages.map((msg) =>
          msg.role === "user" ? (
            <div key={msg.id} className="flex justify-end">
              <div className="max-w-[85%] rounded-2xl rounded-tr-none bg-indigo-600 px-3.5 py-2.5 shadow-sm shadow-indigo-600/10">
            ...
            <div key={msg.id} className="flex flex-col gap-2">
              ...
              <div className="max-w-[92%] rounded-2xl rounded-tl-none bg-white border border-slate-100 shadow-sm px-3.5 py-3">
                <div className="flex items-center gap-1.5 mb-2">
                  <div className="flex h-[18px] w-[18px] items-center justify-center rounded bg-indigo-50 border border-indigo-100 shrink-0">
                ...
                {mode === "workspace" && msg.context?.matchedStepTitle && (
                  <div className="mt-2.5 pt-2 border-t border-slate-100/80">
                    <span className="inline-flex items-center gap-1 rounded-full bg-indigo-50 border border-indigo-100/60 px-2 py-0.5 text-[8px] font-bold text-indigo-600">
                ...
                <div className="space-y-1.5 max-w-[92%]">
                  ...
                  {msg.suggestedQuestions.map((q, i) => (
                    <button
                      ...
                      className="flex w-full items-start gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-left text-[10px] font-medium text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 transition-all group shadow-xs"
                    >
                      <ChevronRight className="h-3 w-3 text-slate-400 group-hover:text-indigo-500 shrink-0 mt-0.5" />
      ...
      <div className="px-3.5 pb-3 pt-2 shrink-0 border-t border-slate-100 bg-white">
        <div className="relative rounded-xl border border-slate-200 bg-slate-50/80 focus-within:bg-white focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/10 transition-all shadow-sm">
          <textarea
            ref={textareaRef}
            rows={2}
            maxLength={MAX_CHARS}
          ...
          <div className="absolute bottom-2 left-3 right-3 flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-lg bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 px-2.5 py-1.5 text-[9px] font-bold text-white shadow-sm shadow-indigo-500/20 disabled:cursor-not-allowed disabled:opacity-40 transition-all"
            >
