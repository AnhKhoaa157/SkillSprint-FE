...
// Đồng bộ hệ thống màu Badge theo template mới của Web
...
export default function AdminUsers() {
  ...
  const [size] = useState(10); // Khớp page size 10 với Dashboard inline
                ...
                {!loading && users.map((u) => {
                  ...
                  return (
                    <tr key={u.id} style={{ borderBottom: "1px solid #F1F5F9" }}
                      ...
                      {/* CỘT HIỂN THỊ THÔNG TIN GÓI DỊCH VỤ ĐĂNG KÝ (TEMPLATE MÀU CAM-TÍM) */}
                      <td style={{ padding: "12px 14px" }}>
                        {sub ? (
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span style={{ color: "#94A3B8", fontSize: "0.7rem" }}>
                              {sub.endDate ? `Hết Hạn: ${new Date(sub.endDate).toLocaleDateString("vi-VN")}` : "Hết Hạn: Vô hạn"}
                            </span>
                          </div>
                        ) : (
                          <span style={{ color: "#94A3B8", fontSize: "0.8rem", fontStyle: "italic" }}>Chưa đăng ký</span>
                        )}
                      </td>
