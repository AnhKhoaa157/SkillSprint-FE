...
function MarketplaceReviewDetail({ itemId }: { itemId: string }) {
  ...
  useEffect(() => { void load(); }, [load]);
  const decide = async () => { if (!action || !item) return; setSaving(true); try { await updateMarketplaceReviewStatus(item.itemId, { status: action, reviewNote: note.trim() || undefined }); toast.success(action === "PUBLISHED" ? "Đã xuất bản Quiz Pack." : action === "REJECTED" ? "Đã từ chối Quiz Pack." : "Đã tạm ngừng bán Quiz Pack."); setAction(null); await load(); } catch (error) { toast.error(errorText(error)); } finally { setSaving(false); } };

  if (loading) return <div className="p-7"><SkeletonRows /></div>;
  if (failed || !item) return <div className="p-7"><ErrorState retry={load} /></div>;

  const actions = item.status === "PENDING_REVIEW" ? <><button onClick={() => setAction("PUBLISHED")} className="inline-flex h-11 items-center justify-center rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-md active:translate-y-0"><CheckCircle2 className="mr-2 h-4 w-4" />Phê duyệt & xuất bản</button><button onClick={() => setAction("REJECTED")} className="inline-flex h-11 items-center justify-center rounded-xl border border-rose-200 bg-white px-4 text-sm font-bold text-rose-700 transition hover:border-rose-300 hover:bg-rose-50"><XCircle className="mr-2 h-4 w-4" />Từ chối</button></> : item.status === "PUBLISHED" ? <button onClick={() => setAction("SUSPENDED")} className="inline-flex h-11 items-center justify-center rounded-xl bg-slate-900 px-4 text-sm font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-700 hover:shadow-md active:translate-y-0"><AlertTriangle className="mr-2 h-4 w-4" />Tạm ngừng bán</button> : null;
